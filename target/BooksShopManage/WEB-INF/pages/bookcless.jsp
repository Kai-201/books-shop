    ...
    <script>
        $(function () {
            layui.use('element', function(){
             ...
             var tbody = $("#tbody");
            var a = 0;
            <c:forEach var="b" items="${bsBookclassList}">
                 var bsBookclassid = "${b.bsBookclassid}";
                 var bsBookclassname = "${b.bsBookclassname}"

                 var $tr = $(" <tr>\n" +
                "                            <td>"+bsBookclassid+"</td>\n" +
                "                            <td>"+bsBookclassname+"</td>\n" +
                "                            <td><a class=\"layui-btn layui-btn-danger layui-btn-xs\" lay-event=\"del\">删除</a>" +
                "                            <input type=\"hidden\" value='${b.bsBookclassid}'></td>\n" +
                "                        </tr>");
                 tbody.append($tr);
                a++;
            </c:forEach>
            $("#tbody").on('click','.layui-btn.layui-btn-danger.layui-btn-xs',function () {
                let bsBookclassid = $(this).next().val();
                let $tr = $(this).parent().parent();
               let index = $tr.index();
               var data = '{"bsBookclassid":'+bsBookclassid+'}';
                $.ajax({
                    url:"../bsBookclass/delById",
            ...
            layui.use('form', function(){
                ...
                form.on('submit', function(data){
                    ...
                    console.log(data);
                    data =
                        '{"bsBookclassname":"'+data.bsBookclassname+'"}';
                    console.log(data);
                      $.ajax({
                        url:"../bsBookclass/insertOne",
                        contentType:"application/json;charset=UTF-8",
                        data:data,
                        dataType:"json",
                        type:"post",
                ...
                <div class="layui-tab-item">
                     <form class="layui-form" lay-filter="test1" method="post">
                        <div class="layui-form-item">
                            <label class="layui-form-label">图书类别名称*</label>
                            <div class="layui-input-block">
                                <input type="text" name="bsBookclassname" lay-verify="bsBookclassname"
                                       placeholder="请输入图书类别名称"  autocomplete="off"
